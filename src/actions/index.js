...
export const fetchItemsFromTypes = (
  basePath,
  name,
  page,
  itemsPerPage,
  ...
  return (ids.length && !old ? Promise.resolve(ids) : fetchIds(name))
    .then(ids => {
      ...
      const activeIds = ids.slice(itemsPerPage * (page - 1), itemsPerPage * page);
      dispatch(setActiveItems(SET_ACTIVE_ITEMS, activeIds));
      return activeIds;
    })
    .then(i => fetchItems(i))
    .then(items => arrayToObject(items, 'id'))
    .then(items => {
...
export const fetchKids = data => dispatch => {
  if (data.length > 0) {
    ...
    children.length > 0 &&
      fetchItems(children).then(items => {
